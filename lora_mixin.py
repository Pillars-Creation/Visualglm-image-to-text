...
class LoraLinear(nn.Module):
    def __init__(self, original_cls, partition, in_dim, out_dim, r, lora_alpha=1., lora_dropout=0., head_first=False,
                 num_attention_heads=None, hidden_size_per_attention_head=None, qlora=False):
        """
        ...
        if qlora:
            try:
                ...
                raise Exception(
                    'Build 4bit layer failed. You need to install the latest bitsandbytes. Try `pip install bitsandbytes`. If you still meet error after installation, try running `from bitsandbytes.nn import LinearNF4` with python and fix the error.')
        else:
        ...
        self.matrix_B = HackParameterList(
            [nn.Parameter(torch.empty((out_dim // partition, r))) for _ in range(partition)])
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict, missing_keys, unexpected_keys,
        ...
        if prefix + 'weight' in state_dict:
            ...
            self.original._load_from_state_dict(state_dict, prefix, local_metadata, strict, missing_keys,
...
def merge_linear_lora(lin):
    if lin.original.weight.data.dtype is not torch.uint8:
        ...
        weight = F.dequantize_fp4(lin.original.weight.data, lin.original.weight.quant_state).to(
            lin.original.bias.data.dtype)
        out_dim, in_dim = weight.shape
...
class LoraMixin(BaseMixin):
    ...
    def reinit(self, parent_model):
        for i in self.layer_range:
            ...
            parent_model.transformer.layers[i].attention.dense = replace_linear_with_lora(
                parent_model.transformer.layers[i].attention.dense, 1, self.r, self.lora_alpha, self.lora_dropout,
                qlora=self.qlora)
            parent_model.transformer.layers[i].attention.query_key_value = replace_linear_with_lora(
                parent_model.transformer.layers[i].attention.query_key_value, 3, self.r, self.lora_alpha,
                self.lora_dropout, head_first=self.head_first, num_attention_heads=self.num_attention_heads,
            ...
            if self.cross_attention and parent_model.transformer.layers[i].is_decoder:
                ...
                parent_model.transformer.layers[i].cross_attention.dense = replace_linear_with_lora(
                    parent_model.transformer.layers[i].cross_attention.dense, 1, self.r, self.lora_alpha,
                    self.lora_dropout, qlora=self.qlora)
                parent_model.transformer.layers[i].cross_attention.query = replace_linear_with_lora(
                    parent_model.transformer.layers[i].cross_attention.query, 1, self.r, self.lora_alpha,
                    self.lora_dropout, qlora=self.qlora)
                parent_model.transformer.layers[i].cross_attention.key_value = replace_linear_with_lora(
                    parent_model.transformer.layers[i].cross_attention.key_value, 2, self.r, self.lora_alpha,
                    self.lora_dropout, qlora=self.qlora)
        if self.qlora:
    ...
    def merge_lora(self):
        for i in self.layer_range:
            ...
            self.transformer.layers[i].attention.query_key_value = merge_linear_lora(
            ...
            if self.transformer.layers[i].is_decoder:
                ...
                self.transformer.layers[i].cross_attention.dense = merge_linear_lora(
                    self.transformer.layers[i].cross_attention.dense)
                self.transformer.layers[i].cross_attention.query = merge_linear_lora(
                    self.transformer.layers[i].cross_attention.query)
                self.transformer.layers[i].cross_attention.key_value = merge_linear_lora(
